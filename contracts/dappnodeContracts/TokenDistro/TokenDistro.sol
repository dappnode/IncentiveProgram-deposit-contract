 ...
 * changelog respect TokenDistro:
 *      * Added `mintAllocationTokens` to allow mint new allocation tokens
 *      * Added `allocateMany` to allow allocate in batches
 */
    ...
    AccessControlEnumerableUpgradeable
{
     ...
     */
    event MintAllocation(uint256 amount);
     ...
     *
     */
    function allocate(address recipient, uint256 amount) public override {
        require(
    ...
    /**
     * Function that allows a distributor to allocate to multiple recipients
        ...
        external
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            allocate(recipients[i], amounts[i]);
        }
    }

    /**
     * Function that allows the DEFAULT_ADMIN_ROLE to add on-top new tokens to distribute
     *
     * Emits a {MintAllocation} event.
     *
     */
    function mintAllocationTokens(uint256 mintAmount) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "TokenDistro::mintAllocationTokens: ONLY_ADMIN_ROLE"
        );

        // Calculate the new tokens added to the initialAmount
        uint256 _mintInitialAmount = (mintAmount * initialAmount) / totalTokens;
        initialAmount += _mintInitialAmount;

        // The rest of the tokens will be added to the locked amount
        lockedAmount += mintAmount - _mintInitialAmount;

        // Add minted tokens to the totalTokens and the allocatedTokens balance
        totalTokens += mintAmount;
        balances[address(this)].allocatedTokens += mintAmount;

        emit MintAllocation(mintAmount);
    }
